...
- **Individual Channel Noise Floors**: Re-calibrated current thresholds individually per channel (Socket 1 & 2 = `0.08A`, Socket 3 & 4 = `0.15A`, Main = `0.08A`) to completely reject the low-level ADC and relay switching noise on general-purpose Sockets 3 & 4, while maintaining accurate 20W load detection on Socket 2.
...
- **PIR Edge Detection & Auto-Clear**: Implemented software edge-detection for the PIR motion sensor. When motion is detected, the dashboard state flashes "Detected" for 3 seconds and then auto-clears to "Clear", while the physical bulb is kept ON for its 15-second stay-on timer. Manual bulb toggles from the dashboard correctly reset the automation timer.